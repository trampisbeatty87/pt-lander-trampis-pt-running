...
// Optional anchor id on a section, so nav links like "#why-us" can jump to it.
function idAttr(b) {
  return b && b.id ? ` id="${esc(b.id)}"` : "";
}
...
const BLOCKS = {
  ...
  text(b, ctx) {
    ...
    return `<section class="section${b.soft ? " soft-bg" : ""}"${idAttr(b)}>
  ...
  steps(b) {
    const items = (b.steps || [])
      .map((s, i) => {
        // With a photo: image card + number badge. Without: a clean numbered circle.
        const media = s.image
          ? `<div class="step-img" style="background-image:url('${esc(s.image)}')"><span class="step-num">${i + 1}</span></div>`
          : `<div class="step-badge"><span class="step-num">${i + 1}</span></div>`;
        return `<div class="step${s.image ? "" : " step-noimg"}">
          ${media}
          <h3>${esc(s.title)}</h3>
          <p>${esc(s.body)}</p>
        </div>`;
  ...
  // Reviews section. Three modes, driven by content.json:
  //   - b.widgetHtml : raw embed code for a live reviews widget (Elfsight/Trustindex/etc.) — injected verbatim
  //   - b.items[]    : manually curated review cards
  //   - b.googleUrl  : adds a "Read our Google reviews" link-out to the clinic's profile
  // If none of the above are present, the whole section is omitted (no empty grid).
  testimonials(b) {
    const list = b.items || [];
    const hasWidget = !!b.widgetHtml;
    const hasGoogle = !!b.googleUrl;
    if (!list.length && !hasWidget && !hasGoogle) return "";
    ...
    const cards = list
      .map(
        (it) => `<figure class="review">
          ...
          ${it.name ? `<figcaption>— ${esc(it.name)}</figcaption>` : ""}
        </figure>`
      )
      .join("\n        ");

    // Widget takes precedence over cards when both are somehow present.
    const body = hasWidget
      ? `<div class="reviews-widget">${b.widgetHtml}</div>`
      : list.length
    ...
    const googleLink = hasGoogle
      ? `<div class="center reviews-cta">
        <a class="btn btn-outline" href="${esc(b.googleUrl)}" target="_blank" rel="noopener">${esc(b.googleCta || "Read our Google reviews ★")}</a>
      </div>`
      : "";
